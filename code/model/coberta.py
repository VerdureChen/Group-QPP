...
from transformers import BertForTokenClassification, BertModel, BertConfig, BertForSequenceClassification, AutoModel, \
...
class COBERT(torch.nn.Module):
    def __init__(self, pre_path_QT, pre_path_QB, pre_path_attn, num_labels, fold=None):
        ...
        self.QT_BERT = AutoModel.from_pretrained(pre_path_QT, num_labels=num_labels)
        self.config1 = AutoConfig.from_pretrained(pre_path_QT)
    ...
    def forward(self, QT_input_ids, QT_token_type_ids=None, QT_attention_masks=None, logit_labels=None, position_ids=None, args=None, ql_scores=None):
        ...
        if self.config1.model_type=="roberta":
            QT_output = self.QT_BERT(input_ids=QT_input_ids,
                                     attention_mask=QT_attention_masks)
        else:
            QT_output = self.QT_BERT(input_ids=QT_input_ids, token_type_ids=QT_token_type_ids,
                                     attention_mask=QT_attention_masks)
        # QT_logits = QT_output.logits
        # sentence_embeddings = self.mean_pooling(QT_output, QT_attention_masks)
        # QT_pooled_output = QT_output.hidden_states[-1][:,0]
        sentence_embeddings = QT_output[0][:,0]
        QT_pooled_output =sentence_embeddings
